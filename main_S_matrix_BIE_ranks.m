function main_S_matrix_BIE_ranks
...
%[A_corr,C,ww,xxsrc,xxtrg] = LOCAL_construct_A_corr(Npan,10,nsrc,ntrg,params,flag_geom,flag_pot);
[A,C,ww,xxsrc,xxtrg] = LOCAL_construct_A_diag(Npan,10,nsrc,ntrg,params,flag_geom,flag_pot);

%%% Compute the Dirichlet data and the solution.
qqsrc  = randn(nsrc,1);
uu_dir = LOCAL_ref_field(C([1,4],:),xxsrc,qqsrc,params,ww);
uu_ref = LOCAL_ref_field(xxtrg,     xxsrc,qqsrc,params,ones(1,length(xxtrg)));
...
uu     = LOCAL_evalpot(xxtrg,C,qq,params,ww,flag_pot);
errmax = max(abs(uu - uu_ref));
errmsq = sqrt(sum((abs(uu - uu_ref).^2))/length(uu));

%%% Evaluate the integral of the boundary potential:
int_qq = sqrt(ww)*qq;
fprintf(1,'flag_pot       errmax       errmsq      int_qq   sum(qqsrc)\n');
fprintf(1,'      %2s   %10.2e   %10.2e   %9.6f   %9.6f\n',...
        flag_pot,errmax,errmsq,int_qq,sum(qqsrc));

nbox_max = 50;
ntot     = size(C,2);
acc      = 1e-10;
NODES    = LOCAL_get_tree_and_lists(linspace(0,1,ntot),nbox_max,0,1);

% figure(1)
% plot(C(1,[1:end,1]),C(4,[1:end,1]),'k','LineWidth',2)
% axis equal
% axis off
% %print('-depsc',sprintf('fig_S_matrix_BIE_ranks_ex1_geom.eps'))
% %export_fig(sprintf('fig_S_matrix_BIE_ranks_ex1_geom.pdf'),'-pdf','-transparent')
% keyboard
% 
% figure(1)
% textshift = 3;
% hold off
% plot(ntot*[0,1,1,0,0],ntot*[0,0,1,1,0],'k')
% hold on
% for pbox = 1:size(NODES,2)
%   if (NODES{5,pbox} == 2)
%     ibox = NODES{4,pbox}(1);
%     jbox = NODES{4,pbox}(2);
%     indi = NODES{6,ibox} - 1 + (1:NODES{7,ibox});
%     i1   = NODES{6,ibox};
%     i2   = NODES{6,ibox} - 1 + NODES{7,ibox};
%     indj = NODES{6,jbox} - 1 + (1:NODES{7,jbox});
%     j1   = NODES{6,jbox};
%     j2   = NODES{6,jbox} - 1 + NODES{7,jbox};
%     B    = A(indi,indj);
%     k    = sum(svd(B) > acc);
%     plot([i1-1,i2,i2,i1-1,i1-1],...
%          [j1-1,j1-1,j2,j2,j1-1],'k',...
%          [j1-1,j1-1,j2,j2,j1-1],...
%          [i1-1,i2,i2,i1-1,i1-1],'k')
%     text(0.5*(i1+i2)-0.5-textshift,0.5*(j1+j2)-0.5,sprintf('%d',k),'Color','b')
%     text(0.5*(j1+j2)-0.5-textshift,0.5*(i1+i2)-0.5,sprintf('%d',k),'Color','b')
%   end
% end
% title(sprintf('Rank structure of A. acc=%8.2e  ntot=%d',acc,ntot))
% axis ij
% axis equal
% axis off
% %print('-depsc',sprintf('fig_S_matrix_BIE_ranks_ex1_ranks.eps'))
% %export_fig(sprintf('fig_S_matrix_BIE_ranks_ex1_ranks.pdf'),'-pdf','-transparent')
% keyboard

figure(1)
hold off
nhalf = floor(ntot/2);
B = A(1:nhalf,(nhalf+1):end);
semilogy(svd(B),'b-','LineWidth',2)
legend('svd(A_{2,3})')
%print('-depsc',sprintf('fig_S_matrix_BIE_ranks_ex1_svd.eps'))
%export_fig(sprintf('fig_S_matrix_BIE_ranks_ex1_svd.pdf'),'-pdf','-transparent')

% keyboard
%%
fprintf("%d by %d \n", size(B,1), size(B,2))
target = TargetMatGenerator(B);
name = fullfile(pwd, sprintf('%s_%s%dx%d.mat', 'target', 'laplace', size(B,1), size(B,2)));
if exist(name,'file')==2
    fprintf('%s exists and updated \n', name)
end
save(name, '-struct', 'target');
fprintf('write out: %s \n', name);
%%
return
...
%%% Create the "correction matrix" and the full matrix.
[A_corr,C,ww,xxsrc,xxtrg] = LOCAL_construct_A_corr(Npan,10,nsrc,ntrg,params,flag_geom,flag_pot);
[A,~,~,~,~,]              = LOCAL_construct_A_diag(Npan,10,nsrc,ntrg,params,flag_geom,flag_pot);
radius = sqrt(max(C(1,:).^2 + C(4,:).^2));
fprintf(1,'Helmholtz combined field. kh=%0.3f  diameter=%0.3f lambda\n',...
        kh,2*radius/lambda)
...
% axis equal
% axis off
% %print('-depsc',sprintf('fig_S_matrix_BIE_ranks_ex3_geom.eps'))
% %export_fig(sprintf('fig_S_matrix_BIE_ranks_ex3_geom.pdf'),'-pdf','-transparent')
% keyboard
% 
% figure(1)
% textshift = 3;
...
% axis equal
% axis off
% %print('-depsc',sprintf('fig_S_matrix_BIE_ranks_ex3_ranks.eps'))
% %export_fig(sprintf('fig_S_matrix_BIE_ranks_ex3_ranks.pdf'),'-pdf','-transparent')
% keyboard
% 
figure(1)
hold off
...
semilogy(svd(B),'b-','LineWidth',2)
legend('svd(A_{2,3})')
axis([0,nhalf,1e-17,1e1])
%print('-depsc',sprintf('fig_S_matrix_BIE_ranks_ex3_svd.eps'))
%export_fig(sprintf('fig_S_matrix_BIE_ranks_ex3_svd.pdf'),'-pdf','-transparent')

% keyboard
%%
fprintf("%d by %d \n", size(B,1), size(B,2))
target = TargetMatGenerator(B);
name = fullfile(pwd, sprintf('%s_%s%dx%d.mat', 'target', 'hemholtz', size(B,1), size(B,2)));
if exist(name,'file')==2
    fprintf('%s exists and updated \n', name)
end
save(name, '-struct', 'target');
...
function NODES = LOCAL_get_tree_and_lists(xx,nbox_max,x_left,x_right)
...
TREE{1,1} = [x_left,x_right]; % Set the geometry.
TREE{2,1} = 0;                % Set the level.
TREE{3,1} = NaN;              % Set the father.
TREE{4,1} = [];               % Set the list of children.
TREE{5,1} = 0;                % Set the number of children.
TREE{6,1} = 1;                % Set the index of the first node.
TREE{7,1} = ntot;             % Set the index of the last node.  
ibox_last = 0;
ncreated  = 1;
...
while (ncreated > 0)
  ...
  for ibox = ibox_first:ibox_last
    ...
    if (nbox > nbox_max)
      xleft              = TREE{1,ibox}(1);
      xright             = TREE{1,ibox}(2);
      xmid               = 0.5*(xleft + xright);
      ind                = TREE{6,ibox} - 1 + (1:TREE{7,ibox});
      nhalf              = sum(xx(ind) <= xmid);
      if (nhalf > 0) % Create the left child.
        ison         = ibox_last + ncreated + 1;
        TREE{4,ibox} = [TREE{4,ibox},ison]; % Update the child list for ibox.
        TREE{5,ibox} = TREE{5,ibox} + 1;    % Update the child list for ibox.
        TREE{1,ison} = [xleft,xmid];
        TREE{2,ison} = TREE{2,ibox} + 1;
        TREE{3,ison} = ibox;
        TREE{4,ison} = [];
        TREE{5,ison} = 0;
        TREE{6,ison} = TREE{6,ibox};
        TREE{7,ison} = nhalf;
        ncreated     = ncreated + 1;
      end
      if (nhalf < TREE{7,ibox})
        ison         = ibox_last + ncreated + 1;
        TREE{4,ibox} = [TREE{4,ibox},ison]; % Update the child list for ibox.
        TREE{5,ibox} = TREE{5,ibox} + 1;    % Update the child list for ibox.
        TREE{1,ison} = [xmid,xright];
        TREE{2,ison} = TREE{2,ibox} + 1;
        TREE{3,ison} = ibox;
        TREE{4,ison} = [];
        TREE{5,ison} = 0;
        TREE{6,ison} = TREE{6,ibox} + nhalf;
        TREE{7,ison} = TREE{7,ibox} - nhalf;
        ncreated     = ncreated + 1;
      end
    end
  end
...
% The operation TREE = TREE{:,1:nboxes} apparently
...
%         These require the construction of two auxiliary lists:
%           leaf_list = a single vector hold all leaves, sorted.
%           nei_lists{ibox} = all neighbors of ibox
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Construct "leaf_list".
leaf_list  = zeros(1,nboxes);
leftpoints = zeros(1,nboxes);
ileaf      = 0;
for ibox = 1:nboxes;
  if (NODES{5,ibox}==0)
    ileaf             = ileaf+1;
    leaf_list(ileaf)  = ibox;
    leftpoints(ileaf) = NODES{1,ibox}(1);
  end
end
nleaves     = ileaf;
[trash,ind] = sort(leftpoints(1:nleaves));
leaf_list   = leaf_list(ind);

% Construct "nei_lists".
nei_lists = cell(1,nboxes);
nei_lists{1} = []; % The root has no neighbors;
for ibox = 2:nboxes
  nei_lists{ibox} = [];
  ifath = NODES{3,ibox};
  for kbox = sort([ifath,nei_lists{ifath}]) % Loop over the father and his neighbors.
    for jbox = NODES{4,kbox} % jbox is a POTENTIAL neighbor of ibox
      if LOCAL_isneighbor(NODES,ibox,jbox)
        nei_lists{ibox} = [nei_lists{ibox},jbox];
      end
    end
  end
end

% Create lists 1 and 5
for j = 1:(nleaves-1)
  ibox_left  = leaf_list(j  );
  ibox_right = leaf_list(j+1);
  if (NODES{1,ibox_left}(2) == NODES{1,ibox_right}(1))
    % ibox_left and ibox_right are indeed neighbors.
    if (NODES{2,ibox_left} > NODES{2,ibox_right})
      % ibox_right is on a COARSER level.
      NODES{18,ibox_left } = [NODES{18,ibox_left },ibox_right];
      NODES{10,ibox_right} = [NODES{10,ibox_right},ibox_left ];
    elseif (NODES{2,ibox_left} < NODES{2,ibox_right})
      % ibox_right is on a FINER level.
      NODES{10,ibox_left } = [NODES{10,ibox_left },ibox_right];
      NODES{18,ibox_right} = [NODES{18,ibox_right},ibox_left ];
    else
      % ibox_right is on the SAME level.
      NODES{10,ibox_left } = [NODES{10,ibox_left },ibox_right];
      NODES{10,ibox_right} = [NODES{10,ibox_right},ibox_left ];
    end
  end
...
for ibox = 2:nboxes
  NODES{12,ibox} = [];
  ifath = NODES{3,ibox};
  for ifathnei = nei_lists{ifath}
    for jbox = NODES{4,ifathnei}
      if LOCAL_iswellsep(NODES,ibox,jbox)
...
% Create lists 3 and 4
% Specifically, for every leaf "ibox", find list3(ibox).
% Creation of list4 is piggybacked as it is the dual of list3.
for ibox = 1:nboxes
  NODES{14,ibox} = [];
  NODES{16,ibox} = [];
end
for ibox = leaf_list
  boxes_to_check = zeros(1,50);
  ntocheck       = 0;
  for jbox = nei_lists{ibox}
    boxes_to_check(ntocheck + (1:NODES{5,jbox})) = NODES{4,jbox};
    ntocheck = ntocheck + NODES{5,jbox};
  end
  while (ntocheck > 0)
    jbox = boxes_to_check(1);
    if LOCAL_iswellsep(NODES,jbox,ibox)
      NODES{14,ibox} = [NODES{14,ibox},jbox];
      NODES{16,jbox} = [NODES{16,jbox},ibox];
    else
      boxes_to_check(ntocheck + (1:NODES{5,jbox})) = NODES{4,jbox};
      ntocheck = ntocheck + NODES{5,jbox};
    end
    boxes_to_check(1) = boxes_to_check(ntocheck);
    ntocheck = ntocheck-1;
  end
end
...
function output = LOCAL_isneighbor(NODES,ibox,jbox)

if (NODES{2,ibox} == NODES{2,jbox}) % ibox and jbox are on the same level
  if (NODES{1,ibox}(2) == NODES{1,jbox}(1)) 
    output = 1;
  elseif (NODES{1,ibox}(1) == NODES{1,jbox}(2)) 
    output = 1;
  else
    output = 0;
  end
else % ibox and jbox are not on the same level
  output = 0;
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This function checks if jbox is well-separated from ibox

function output = LOCAL_iswellsep(NODES,ibox,jbox)

xleft  = NODES{1,ibox}(1);
xright = NODES{1,ibox}(2);
width  = xright - xleft;
xmid   = 0.5*(xleft+xright);
dist   = min(abs(NODES{1,jbox} - xmid));
if (dist >= (1.499999999999*width))
  output = 1;
else
  output = 0;
end
...
function main_laplace_exterior
...
fprintf(1,'flag_pot  err_submat       errmax       errmsq      int_qq   sum(qqsrc)\n');
FLAGS = ['ls';'ds'];
...
for ipot = 1:size(FLAGS,1)
  ...
  ind1 = [24:26,33];
  ind2 = [33:36,75];
  err_submat = max(max(abs(A(ind1,ind2) - LOCAL_A_submat(C,ww,ind1,ind2,A_corr,flag_pot,params))));
  ...
  %%% Evaluate the integral of the boundary potential:
  int_qq = sqrt(ww)*qq;
  fprintf(1,'      %2s  %10.2e   %10.2e   %10.2e   %9.6f   %9.6f\n',...
          flag_pot,err_submat,errmax,errmsq,int_qq,sum(qqsrc));
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [C,xxint,xxext] = LOCAL_construct_cont(tt,nint,next,flag_geom)
...
if strcmp(flag_geom,'star')
  ...
  k        = 5;
  C        = zeros(6,ntot);
  C(1,:)   =   1.5*              cos(2*pi*tt) + (r/2)*                          cos((k+1)*2*pi*tt) + (r/2)*cos((k-1)*2*pi*tt);
  C(2,:)   = - 1.5*(2*pi)*       sin(2*pi*tt) - (r/2)*(k+1)*(2*pi)*             sin((k+1)*2*pi*tt) - (r/2)*(k-1)*(2*pi)*sin((k-1)*2*pi*tt);
  C(3,:)   = - 1.5*(2*pi)*(2*pi)*cos(2*pi*tt) - (r/2)*(k+1)*(k+1)*(2*pi)*(2*pi)*cos((k+1)*2*pi*tt) -...
              (r/2)*(k-1)*(k-1)*(2*pi)*(2*pi)*cos((k-1)*2*pi*tt);
  C(4,:)   =                 sin(2*pi*tt) + (r/2)*                          sin((k+1)*2*pi*tt) - (r/2)*sin((k-1)*2*pi*tt);
  C(5,:)   =   (2*pi)*       cos(2*pi*tt) + (r/2)*(k+1)*(2*pi)*             cos((k+1)*2*pi*tt) - (r/2)*(k-1)*(2*pi)*cos((k-1)*2*pi*tt);
  C(6,:)   = - (2*pi)*(2*pi)*sin(2*pi*tt) - (r/2)*(k+1)*(k+1)*(2*pi)*(2*pi)*sin((k+1)*2*pi*tt) + ...
               (r/2)*(k-1)*(k-1)*(2*pi)*(2*pi)*sin((k-1)*2*pi*tt);

  xxint = [0.5 + 0.1*(rand(1,nint)-0.5);0.1*(rand(1,nint)-0.5)];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [A_corr,C,ww,xxint,xxext] = LOCAL_construct_A_corr(Npan,Ngau,nint,next,params,...
                                                            flag_geom,flag_pot)
...
tt_panels = linspace(0,1,Npan+1);
         
[tt,ww]                   = LOCAL_get_gauss_nodes(tt_panels,Ngau);
[tt_sing,ww_sing]         = LOCAL_get_gauss_singnodes(tt_panels); 
[tt_neighRht,ww_neighRht] = LOCAL_get_gauss_neighRht(tt_panels,tt,t_sta,t_end); 
[tt_neighLft,ww_neighLft] = LOCAL_get_gauss_neighLft(tt_panels,tt,t_sta,t_end);


[C,xxint,xxext]  = LOCAL_construct_cont(tt,nint,next,flag_geom);
ww = ww.*sqrt(C(2,:).^2 + C(5,:).^2);
    
C_sing      = LOCAL_construct_cont(tt_sing,nint,next,flag_geom);
ww_sing     = ww_sing.*sqrt(C_sing(2,:).^2 + C_sing(5,:).^2);

C_neighRht  = LOCAL_construct_cont(tt_neighRht,nint,next,flag_geom);
ww_neighRht = ww_neighRht.*sqrt(C_neighRht(2,:).^2 + C_neighRht(5,:).^2);

C_neighLft  = LOCAL_construct_cont(tt_neighLft,nint,next,flag_geom);
ww_neighLft = ww_neighLft.*sqrt(C_neighLft(2,:).^2 + C_neighLft(5,:).^2);
  
npanels = length(tt_panels)-1; 

ker = cell(npanels, npanels);
A   = zeros(npanels*10, npanels*10);
for row=1:npanels
...
% correct the diagonal panels    
    ...
    singInterpMat = LOCAL_legendre_interp(s, t_sing, w_sing); % cell(10, 1)
    for ii=1:10
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [A,C,ww,xxint,xxext] = LOCAL_construct_A_diag(Npan,Ngau,nint,next,params,...
                                                       flag_geom,flag_pot)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function A12 = LOCAL_A_submat(C,ww,ind1,ind2,A_corr,flag_pot,params)
...
aa      = reshape(full(A_corr(ind1,ind2)),1,n1*n2);
...
if(strcmp(flag_pot,'ls'))    
  aa(indreg) = -1/(4*pi)*log(dd1.*dd1 + dd2.*dd2);

elseif(strcmp(flag_pot,'ds'))   
  ddsq = dd1.*dd1 + dd2.*dd2;  
  aa(indreg) = -1/(2*pi)*(nn1.*dd1 + nn2.*dd2)./ddsq ...
        -1/(4*pi)*log(ddsq);

elseif(strcmp(flag_pot,'hs'))
  dd         = sqrt(dd1.*dd1 + dd2.*dd2);
  aa(indreg) = ima/4*besselh(0, kh*dd);

elseif(strcmp(flag_pot,'hd'))
  dd         = sqrt(dd1.*dd1 + dd2.*dd2);
  aa(indreg) = (nn1.*dd1 + nn2.*dd2).*(1./dd).*(-kh*besselh(1, kh*dd))*ima/4;
...
aa(indreg)  = sqrt(ww(ii(indreg)).*ww(jj(indreg))).*aa(indreg);  
A12 = reshape(aa,n1,n2);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function uu = LOCAL_ref_field(xxtrg,xxsrc,qq,params,wwtrg)
...
if (kh == 0) % Laplace problem
  uu = -1/(4*pi)*log(DD)*reshape(qq,numel(qq),1).*sqrt(wwtrg.');
else % Helmholtz problem.
  uu = ima/4*besselh(0,kh*sqrt(DD))*reshape(qq,numel(qq),1).*sqrt(wwtrg.');
end

return

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function vv = LOCAL_evalpot(xx,C,uu,params,ww,potname)
